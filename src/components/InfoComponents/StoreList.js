...
export default function StoreList({
    ...
    const getOpenCloseTime = (store) => {
        if (store.open_time == undefined && store.close_time == undefined) return '운영정보없음';
            ...
            <ListSection>
                ...
                <SearchStore>
                    <Input
                        ...
                        onKeyPress={(e) => {
                            if (e.key === 'Enter') filterStoreName(e.target.value);
                        }}
                    />
                    <SearchIcon src={'https://static.koreatech.in/assets/img/search.png'} />
                </SearchStore>
                {/* Category 밑에 있는 것들 */}
                {children}
                <FilterWrapper>
                    ...
                    <CheckboxWrapper>
                        <Checkbox id="card" checked={filter & 2} onChange={() => selectFilter(1)} />
                        <CheckboxLabel htmlFor="card">카드결제 가능</CheckboxLabel>
                    </CheckboxWrapper>
                    <CheckboxWrapper>
                        <Checkbox id="bank" checked={filter & 4} onChange={() => selectFilter(2)} />
                ...
                <ListWrapper>
                    {filteredStoreList.map((store, index) => (
                        <ListItem to={`/store/${store.permalink}`} index={index} key={store.id}>
                            {isStoreOpen(store.open_time, store.close_time) && <NotOpenAlert />}
